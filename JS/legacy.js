...
function appendData(data) {
  ...
  for (const key in data) {
    ...
    if(passedFinalEpilogue == true){
      ...
      if(key == "AAAAAAAAAAAAAAAAAAAA"){
        ...
        div.innerHTML = `
          <button type="button" class = "collapsible">
            ...
            <div class = "title">
            <h2 class = "Ax20">${counter}. ${level.name}<br><i><span style="font-size: 0.6em;">by ${level.publisher}</span></i></h2>
        ...
        div.innerHTML = `
          <button type="button" class = "collapsible">
            ...
            <h3>Creator Points: ${level.creatorpoints}</h3>
            </div>
            </section>
            </button>
        `;
      ...
      for (let i = 0; i < 1; i++) {
        let victor = document.createElement("div");
        if (level.ytcode == "5JXq2mdNjbU"){
        victor.innerHTML = `
          <h5>Verified by ${level.verifier}<br>ID: ${level.id} <br>Video of level: <a href = "${level.list[i].link}" target = "_blank">here</a></h5>
            <h6>Legacy list levels do not accept records.</h6>
        `;
        }
        else{
        victor.innerHTML = `
          <h5>Verified by ${level.verifier}<br>ID: ${level.id}</h5>
            <h6>Legacy list levels do not accept records.</h6>
        `;
        }
        listOfVictors.appendChild(victor);
