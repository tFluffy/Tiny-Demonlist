...
function appendData(data) {
    ...
    for (const key in data) {
      ...
      if(aspectRatio < 1 && (level.name == "meowmeowmeowmeowmeow" ||
      level.name == "marethenightbypvbz" ||
      level.name == "seashellsellingsally" ||
      level.name == "WMWMWMWMWMWMWMWMWMWM")){
          ...
          div.innerHTML = `
            ...
            <h3>Score: ${(250 / (Math.pow(1.05337008, 0.37544272 * (counter - 1)) * Math.pow(counter, 0.0619178))).toFixed(3)}</h3>
            </div>
            </section>
            </button>
          `;
      ...
      for (let i = 0; i < level.list.length; i++) {
        ...
        if(level.victors == "0"){
          victor.innerHTML = `
          <h5>Verified by ${level.verifier} in ${level.verifierTime} seconds<br>Creator Points: ${level.creatorpoints}<br>ID: ${level.id}<hr>Completions:<br></h5>
